    ...
    for (machine_type, lot), group in grouped:
        ...
        while True:
            ...
            for route in selected_routes:
                ...
                all_used_indices.add(route.name)

            merged_routes.append(merge_rows(selected_routes))

    # Добавим оставшиеся неиспользованные маршруты (как есть, без объединения)
    remaining_routes = df[~df.index.isin(all_used_indices)]
    for _, row in remaining_routes.iterrows():
        merged_routes.append(row)
